...
public class ApiRootController(TwitchAPI twitchApi, ApplicationDbContext dbContext, TwitchTokensManager twitchTokensService, ILogger logger) : Controller {
    ...
    [HttpPost]
    public async Task<ActionResult<TwitchManagedReward>> Post([FromRoute] string userId, [FromBody] CreateCustomRewardsRequest customRewardsRequest) {
        logger.Error("{@e}", customRewardsRequest);
